...
#table = driver.find_element(By.CLASS_NAME, tableClass)
#table = driver.find_element(By.TAG_NAME, tableTag)
...
for i in range(len(rows)):
    ...
    if len(firstColumn.text) == 0: #rows[i].is_displayed(): I'd like to use this function but it seems to think some visible rows aren't visible.
        ...
        print("Clicking preveious carat to expand table")
    ...
    try:
        ...
        #This works but it removes the entire text field not just the pop up. Might have to find soluction that just sets the popup to none.
